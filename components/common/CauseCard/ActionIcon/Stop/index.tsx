import React, { FC, useState } from "react";
import Link from "next/link";
import { USER_CAUSES_PATH } from "helpers/paths";
import { useSelector, useDispatch } from "react-redux";
import { Form, Button, Modal } from "antd";
import { CloseOutlined } from "@ant-design/icons";
import styles from "./stop.module.scss";
import { IRootState } from "redux/initialStates";
import { validateMessages } from "constants/validationMessages";
import { InputPassword } from "components/common/Input";
import stopCause from "redux/actions/cause/stopCause";
import { Store } from "antd/lib/form/interface";

interface IStopCauseProps {
  slug: string;
  visible: boolean;
  onCancel: () => void;
}

const StopCause: FC<IStopCauseProps> = ({ slug, visible, onCancel }) => {
  const dispatch = useDispatch();
  const [submit, setSubmit] = useState<boolean>(false);
  const [stopSuccessful, setStopSuccessful] = useState<boolean>(false);
  const { loading } = useSelector(({ cause: { stop } }: IRootState) => stop);

  const handleSubmit = (data: Store) => {
    stopCause(slug, data)(setStopSuccessful, dispatch);
  };

  const onChange = (pin: string) => {
    setSubmit(pin.length === 5);
  };

  return (
    <Modal
      visible={visible}
      footer={false}
      closable={false}
      destroyOnClose={true}
      maskStyle={{ background: "#000000b3" }}
    >
      {!stopSuccessful && (
        <div className={styles.stop__modalHeader}>
          <CloseOutlined
            className={styles.stop__modalHeader__icon}
            onClick={onCancel}
          />
        </div>
      )}

      <div className={styles.stop__modalContent}>
        <h4>{stopSuccessful ? "Cause Stopped" : "Stop a Cause"}</h4>

        {stopSuccessful ? (
          <div className={styles.stop__modalContent__successful}>
            <img src="/success-stop.gif" alt="stop success" />
            <Link href={USER_CAUSES_PATH}>
              <a onClick={onCancel}>Back Home</a>
            </Link>
          </div>
        ) : (
          <>
            <p className={styles.stop__modalContent__subTitle}>
              By stopping this cause, donours will not be able to donate money
              for it again, already donated money will be remitted to you at the
              end date you already choose.
            </p>
            <p className={styles.stop__modalContent__continue}>
              To Continue Enter Your PIN
            </p>

            <Form
              onFinish={handleSubmit}
              validateMessages={validateMessages}
              className={styles.stop__modalContent__form}
            >
              <Form.Item
                className="form-group"
                rules={[
                  {
                    len: 5,
                    required: true,
                    pattern: /^[0-9]{5}$/,
                    message: "Pin must be number of 5 digits",
                  },
                ]}
                validateTrigger={["onSubmit", "onBlur"]}
                name="password"
              >
                <InputPassword
                  maxLength={5}
                  placeholder="PIN"
                  onChange={(e) => onChange(e.target.value)}
                />
              </Form.Item>
              <div className="d-flex mb-3">
                <Button
                  htmlType="submit"
                  loading={loading}
                  className="btn-primary ml-auto"
                >
                  {submit ? "SUBMIT" : "STOP"}
                </Button>
              </div>
            </Form>
          </>
        )}
      </div>
    </Modal>
  );
};

export default StopCause;
